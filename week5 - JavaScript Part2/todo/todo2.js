...
const todoList2 = {
  ...
  // 새로운 항목을 추가하는 메소드
  add(manager, title, dueDate) {
    const item = new TodoItem(manager, title, dueDate);
  ...
  save() {
    ...
    localStorage.setItem("todoList_teamMode", JSON.stringify(this.items));
  },

  // To-Do 항목 리스트를 로컬 스토리지에서 불러오는 메소드
  load() {
    const items = localStorage.getItem("todoList_teamMode");
    if (items) {
